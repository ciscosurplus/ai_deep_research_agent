...
@function_tool
async def deep_research(query: str, max_depth: int = None, time_limit: int = None, max_urls: int = None, 
                       maxDepth: int = None, timeLimit: int = None, maxUrls: int = None) -> Dict[str, Any]:
    """
    Perform comprehensive web research using Firecrawl's deep research endpoint.
    
    Args:
        query: The research query or topic
        max_depth: Maximum crawl depth (snake_case)
        time_limit: Maximum time limit in seconds (snake_case)
        max_urls: Maximum number of URLs to process (snake_case)
        maxDepth: Maximum crawl depth (camelCase)
        timeLimit: Maximum time limit in seconds (camelCase)
        maxUrls: Maximum number of URLs to process (camelCase)
    """
    try:
        # Handle both snake_case and camelCase parameters
        _max_depth = max_depth if max_depth is not None else maxDepth
        _time_limit = time_limit if time_limit is not None else timeLimit
        _max_urls = max_urls if max_urls is not None else maxUrls
